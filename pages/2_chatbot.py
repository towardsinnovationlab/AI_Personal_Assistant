...
if prompt := st.chat_input():
    ...
    if option == 'Mixtral 8x7B':
        ...
        response = client.chat(model="open-mixtral-8x7b", messages=st.session_state.chatbot.messages)
    elif option == 'GPT-4o':
        client = OpenAI(api_key=api_key)
        response = client.chat.completions.create(model="gpt-4o", messages=st.session_state.chatbot.messages)    
    elif option == 'GPT-3.5-turbo':
        client = OpenAI(api_key=api_key)
        response = client.chat.completions.create(model="gpt-3.5-turbo", messages=st.session_state.chatbot.messages)
    elif option == 'Llama-3-70B':
        client = OpenAI(api_key=api_key,base_url="https://api.llama-api.com")
        response = client.chat.completions.create(model="llama3-70b", messages=st.session_state.chatbot.messages)
    else:
